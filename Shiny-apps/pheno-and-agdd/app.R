 ...
 output$yearsPlot <- renderPlotly({
  ...
  # observeEvent(
     p <- ggplot(data=site_select, aes(x=dayOfYear, y=AGDD, color = as.factor(year),group = 1,
                                       text = paste("</br>Day of Year:",dayOfYear, "</br>AGDDs:",AGDD, "</br>Year:", year))) +
...
# if the user chooses to observe phenophases, then this plot is generated
 output$phenoPlot <- renderPlotly({
  ...
  # legend font stuff
   ...
   #observeEvent(
   # plot the graph 
   p <- ggplot(phenoStat_T, aes(x = dayOfYear, y = percent, fill = phenophaseName, color = phenophaseName,group = 1,
                                text = paste("</br>Day of Year:",dayOfYear, "</br>Percent:",percent, "</br>Phenophase:", phenophaseName))) +
      ...
      theme(legend.position = "bottom") + labs(fill = "", color = "")
   
   # hover over plot
   gp <- ggplotly(p, tooltip = c("text")) %>% config(displayModeBar = FALSE)  %>% 
     layout(legend=list(x=.5,y=-.4, text = "Phenophase", yanchor="left")) %>% layout(legend = l) %>% 
   ...
   gp[['x']][['layout']][['annotations']][[1]][['y']] <- -0.07
