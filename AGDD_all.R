...
# would AGDD be more accurate using the tempSingleMinimum and tempSingleMaximum columns instead of just the tempSingleMean??
...
#calculate Accumlated GDD
# uses the sumr.2 function to add the GDDs together and puts it in AGDD
day_temp_2$AGDD <- sumr.2(x=day_temp_2$GDD)
...
# calculates AGDDs from the beginning of data collection and not just for the specified year
# now plots from beginning of year
AGDD_2017_2 <- ggplot(data=day_temp_2, aes(x=dayOfYear, y=AGDD, group = 1)) +
  geom_path() + xlab("Day of Year") + ggtitle("Aggregated Growing Degree Days in 2017")
AGDD_2017_2
...
# comparing the two different AGDDs for 2017
# looks like the second way makes there be more AGDDs compared to the original way of calculating
grid.arrange(AGDD_2017, AGDD_2017_2)

# comparing the second AGDD with the HARV data in HARV.R
